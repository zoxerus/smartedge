...
def send_cli_command_to_bmv2(cli_command, thrif_ip='0.0.0.0', thrift_port=DEFAULT_THRIFT_PORT, instance=get_this_ap()):
...
def add_bmv2_swarm_broadcast_port(switch_port, ap_ip, thrift_port=DEFAULT_THRIFT_PORT, instance=get_this_ap()):
...
def remove_bmv2_swarm_broadcast_port(switch_port, ap_ip, thrift_port=DEFAULT_THRIFT_PORT, instance=get_this_ap() ):
...
def add_entry_to_bmv2(communication_protocol, table_name, action_name, match_keys, action_params, thrift_ip = '0.0.0.0', thrift_port = DEFAULT_THRIFT_PORT, instance=get_this_ap()):
...
def get_entry_handle(table_name, key, thrift_ip = '0.0.0.0', thrift_port = DEFAULT_THRIFT_PORT, instance=get_this_ap()):
...
def delete_forwarding_entry_from_bmv2(
    communication_protocol, table_name, key, thrift_ip = '0.0.0.0', thrift_port = DEFAULT_THRIFT_PORT, instance=get_this_ap()):
    if communication_protocol == P4_CONTROL_METHOD_THRIFT_CLI:
